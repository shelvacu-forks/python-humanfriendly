...
def assertRaises(
    exception_class: type[Exception], fn: Callable[[], None] | None = None
) -> pytest.RaisesExc | None:
    if fn is None:
...
def test_case_insensitive_dict():
    ...
    assert (
        len(CaseInsensitiveDict([("one", True), ("ONE", False)], one=False, two=True))
        == 2
    )
...
def test_skip_on_raise():
    ...
    assertRaises(NotImplementedError, test_fn)
    assertRaises(unittest.SkipTest, decorated_fn)
...
def test_retry_return():
    ...
    with assertRaises(CallableTimedOut):
        retry(lambda: False, timeout=1)


def test_mocked_program():
...
def test_run_cli_intercepts_exit():
    """Test that run_cli() intercepts SystemExit."""
    returncode, output = run_cli(lambda: sys.exit(42))
    assertEqual(returncode, 42)


def test_run_cli_intercepts_error():
    """Test that run_cli() intercepts exceptions."""
    returncode, output = run_cli(run_cli_raise_other)
    assertEqual(returncode, 1)


def run_cli_raise_other():
    """run_cli() sample that raises an exception."""
    raise ValueError()


def test_run_cli_intercepts_output():
...
def test_generate_slug():
    ...
    assertEqual("some-random-text", generate_slug("Some Random Text!"))
...
def test_boolean_coercion():
    ...
    for value in [True, "TRUE", "True", "true", "on", "yes", "1"]:
        assertEqual(True, coerce_boolean(value))
    for value in [False, "FALSE", "False", "false", "off", "no", "0"]:
        assertEqual(False, coerce_boolean(value))
    with assertRaises(ValueError):
        coerce_boolean("not a boolean")


def test_pattern_coercion():
...
def test_parse_timespan():
    ...
    assertEqual(0, parse_timespan("0"))
    assertEqual(0, parse_timespan("0s"))
    assertEqual(0.000000001, parse_timespan("1ns"))
    assertEqual(0.000000051, parse_timespan("51ns"))
    assertEqual(0.000001, parse_timespan("1us"))
    assertEqual(0.000052, parse_timespan("52us"))
    assertEqual(0.001, parse_timespan("1ms"))
    assertEqual(0.001, parse_timespan("1 millisecond"))
    assertEqual(0.5, parse_timespan("500 milliseconds"))
    assertEqual(0.5, parse_timespan("0.5 seconds"))
    assertEqual(5, parse_timespan("5s"))
    assertEqual(5, parse_timespan("5 seconds"))
    assertEqual(60 * 2, parse_timespan("2m"))
    assertEqual(60 * 2, parse_timespan("2 minutes"))
    assertEqual(60 * 3, parse_timespan("3 min"))
    assertEqual(60 * 3, parse_timespan("3 mins"))
    assertEqual(60 * 60 * 3, parse_timespan("3 h"))
    assertEqual(60 * 60 * 3, parse_timespan("3 hours"))
    assertEqual(60 * 60 * 24 * 4, parse_timespan("4d"))
    assertEqual(60 * 60 * 24 * 4, parse_timespan("4 days"))
    assertEqual(60 * 60 * 24 * 7 * 5, parse_timespan("5 w"))
    assertEqual(60 * 60 * 24 * 7 * 5, parse_timespan("5 weeks"))
    with assertRaises(InvalidTimespan):
        parse_timespan("1z")


def test_parse_date():
    """Test :func:`humanfriendly.parse_date()`."""
    assertEqual((2013, 6, 17, 0, 0, 0), parse_date("2013-06-17"))
    assertEqual((2013, 6, 17, 2, 47, 42), parse_date("2013-06-17 02:47:42"))
    assertEqual((2016, 11, 30, 0, 47, 17), parse_date("2016-11-30 00:47:17"))
    with assertRaises(InvalidDate):
        parse_date("2013-06-XY")


def test_format_size():
    """Test :func:`humanfriendly.format_size()`."""
    assertEqual("0 bytes", format_size(0))
    assertEqual("1 byte", format_size(1))
    assertEqual("42 bytes", format_size(42))
    assertEqual("1 KB", format_size(1000**1))
    assertEqual("1 MB", format_size(1000**2))
    assertEqual("1 GB", format_size(1000**3))
    assertEqual("1 TB", format_size(1000**4))
    assertEqual("1 PB", format_size(1000**5))
    assertEqual("1 EB", format_size(1000**6))
    assertEqual("1 ZB", format_size(1000**7))
    assertEqual("1 YB", format_size(1000**8))
    assertEqual("0 bytes", format_size(0, force_unit="bytes"))
    assertEqual("1 byte", format_size(1, force_unit="bytes"))
    assertEqual("42 bytes", format_size(42, force_unit="bytes"))
    assertEqual("1 KB", format_size(1000**1, force_unit="KB"))
    assertEqual("1 MB", format_size(1000**2, force_unit="MB"))
    assertEqual("1 GB", format_size(1000**3, force_unit="GB"))
    assertEqual("1 TB", format_size(1000**4, force_unit="TB"))
    assertEqual("1 PB", format_size(1000**5, force_unit="PB"))
    assertEqual("1 EB", format_size(1000**6, force_unit="EB"))
    assertEqual("1 ZB", format_size(1000**7, force_unit="ZB"))
    assertEqual("1 YB", format_size(1000**8, force_unit="YB"))
    assertEqual("4 TB", format_size(1000**4 * 4, force_unit="TB"))
    assertEqual("4000 GB", format_size(1000**4 * 4, force_unit="GB"))
    assertEqual("4000000 MB", format_size(1000**4 * 4, force_unit="MB"))
    assertEqual("4000000000 KB", format_size(1000**4 * 4, force_unit="KB"))
    assertEqual("4000000000000 bytes", format_size(1000**4 * 4, force_unit="bytes"))
    assertEqual("1 KiB", format_size(1024**1, binary=True))
    assertEqual("1 MiB", format_size(1024**2, binary=True))
    assertEqual("1 GiB", format_size(1024**3, binary=True))
    assertEqual("1 TiB", format_size(1024**4, binary=True))
    assertEqual("1 PiB", format_size(1024**5, binary=True))
    assertEqual("1 EiB", format_size(1024**6, binary=True))
    assertEqual("1 ZiB", format_size(1024**7, binary=True))
    assertEqual("1 YiB", format_size(1024**8, binary=True))
    assertEqual("1 KiB", format_size(1024**1, binary=True, force_unit="KiB"))
    assertEqual("1 MiB", format_size(1024**2, binary=True, force_unit="MiB"))
    assertEqual("1 GiB", format_size(1024**3, binary=True, force_unit="GiB"))
    assertEqual("1 TiB", format_size(1024**4, binary=True, force_unit="TiB"))
    assertEqual("1 PiB", format_size(1024**5, binary=True, force_unit="PiB"))
    assertEqual("1 EiB", format_size(1024**6, binary=True, force_unit="EiB"))
    assertEqual("1 ZiB", format_size(1024**7, binary=True, force_unit="ZiB"))
    assertEqual("1 YiB", format_size(1024**8, binary=True, force_unit="YiB"))
    assertEqual("4 TiB", format_size(1024**4 * 4, binary=True, force_unit="TiB"))
    assertEqual("4096 GiB", format_size(1024**4 * 4, binary=True, force_unit="GiB"))
    assertEqual("4194304 MiB", format_size(1024**4 * 4, binary=True, force_unit="MiB"))
    assertEqual(
        "4294967296 KiB", format_size(1024**4 * 4, binary=True, force_unit="KiB")
    )
    assertEqual(
        "4398046511104 bytes", format_size(1024**4 * 4, binary=True, force_unit="bytes")
    )
    assertEqual("45 KB", format_size(1000 * 45))
    assertEqual("2.9 TB", format_size(1000**4 * 2.9))


def test_parse_size():
    """Test :func:`humanfriendly.parse_size()`."""
    assertEqual(0, parse_size("0B"))
    assertEqual(42, parse_size("42"))
    assertEqual(42, parse_size("42B"))
    assertEqual(1000, parse_size("1k"))
    assertEqual(1024, parse_size("1k", binary=True))
    assertEqual(1000, parse_size("1 KB"))
    assertEqual(1000, parse_size("1 kilobyte"))
    assertEqual(1024, parse_size("1 kilobyte", binary=True))
    assertEqual(1000**2 * 69, parse_size("69 MB"))
    assertEqual(1000**3, parse_size("1 GB"))
    assertEqual(1000**4, parse_size("1 TB"))
    assertEqual(1000**5, parse_size("1 PB"))
    assertEqual(1000**6, parse_size("1 EB"))
    assertEqual(1000**7, parse_size("1 ZB"))
    assertEqual(1000**8, parse_size("1 YB"))
    assertEqual(1000**3 * 1.5, parse_size("1.5 GB"))
    assertEqual(1024**8 * 1.5, parse_size("1.5 YiB"))
    with assertRaises(InvalidSize):
        parse_size("1q")
    with assertRaises(InvalidSize):
        parse_size("a")
...
def test_format_length():
    ...
    assertEqual("0 metres", format_length(0))
    assertEqual("1 metre", format_length(1))
    assertEqual("42 metres", format_length(42))
    assertEqual("1 km", format_length(1 * 1000))
    assertEqual("15.3 cm", format_length(0.153))
    assertEqual("1 cm", format_length(1e-02))
    assertEqual("1 mm", format_length(1e-03))
    assertEqual("1 nm", format_length(1e-09))


def test_parse_length():
    """Test :func:`humanfriendly.parse_length()`."""
    assertEqual(0, parse_length("0m"))
    assertEqual(42, parse_length("42"))
    assertEqual(1.5, parse_length("1.5"))
    assertEqual(42, parse_length("42m"))
    assertEqual(1000, parse_length("1km"))
    assertEqual(0.153, parse_length("15.3 cm"))
    assertEqual(1e-02, parse_length("1cm"))
    assertEqual(1e-03, parse_length("1mm"))
    assertEqual(1e-09, parse_length("1nm"))
    with assertRaises(InvalidLength):
        parse_length("1z")
    with assertRaises(InvalidLength):
        parse_length("a")
...
def test_format_number():
    ...
    assertEqual("1.56", format_number(1.56789))
    assertEqual("1.567", format_number(1.56789, 3))
    assertEqual("1,000", format_number(1000))
    assertEqual("1,000", format_number(1000.12, 0))
    assertEqual("1,000,000", format_number(1000000))
    assertEqual("1,000,000.42", format_number(1000000.42))
    # Regression test for https://github.com/xolox/python-humanfriendly/issues/40.
    assertEqual("-285.67", format_number(-285.67))


def test_round_number():
    """Test :func:`humanfriendly.round_number()`."""
    assertEqual("1", round_number(1))
    assertEqual("1", round_number(1.0))
    assertEqual("1.00", round_number(1, keep_width=True))
    assertEqual("3.14", round_number(3.141592653589793))
...
def test_format_path():
    ...
    assertEqual(friendly_path, format_path(absolute_path))
...
def test_parse_path():
    ...
    friendly_path = os.path.join("~", ".vimrc")
    absolute_path = os.path.join(os.environ["HOME"], ".vimrc")
    assertEqual(absolute_path, parse_path(friendly_path))
...
def test_concatenate():
    ...
    assert (
        concatenate(["one", "two", "three"], serial_comma=True) == "one, two, and three"
    )
...
def test_split():
    ...
    assertEqual(split(""), [])
    assertEqual(split("foo"), ["foo"])
    assertEqual(split("foo, bar"), ["foo", "bar"])
    assertEqual(split("foo, bar, baz"), ["foo", "bar", "baz"])
    assertEqual(split("foo,bar,baz"), ["foo", "bar", "baz"])
...
def test_timer():
    ...
    for seconds, text in (
        ...
        assertEqual(round_number(t.elapsed_time, keep_width=True), "%i.00" % seconds)
        assertEqual(str(t), text)
    # Test rounding to seconds.
    t = Timer(time.time() - 2.2)
    assertEqual(t.rounded, "2 seconds")
    ...
    assertEqual(normalize_timestamp(automatic_timer.elapsed_time, 0), "1.00")
    # Test resumable timer.
    resumable_timer = Timer(resumable=True)
    for i in range(2):
        with resumable_timer:
            time.sleep(1)
    assertEqual(normalize_timestamp(resumable_timer.elapsed_time, 0), "2.00")
...
def test_spinner():
    ...
    assertTrue(len(lines) > 0)
    assertTrue(all("test spinner" in line for line in lines))
    assertTrue(all("%" in line for line in lines))
    assertEqual(sorted(set(lines)), sorted(lines))


def test_automatic_spinner():
    """
...
def test_prompt_for_choice():
    ...
    with PatchedAttribute(prompts, "interactive_prompt", lambda p: ""):
        with assertRaises(TooManyInvalidReplies):
            prompt_for_choice(["a", "b", "c"])


def test_prompt_for_confirmation(capsys):
    ...
    replies = ["", "y"]
    capsys.readouterr()  # just to clear the captured buffer
    with PatchedAttribute(prompts, "interactive_prompt", lambda p: replies.pop(0)):
        assert prompt_for_confirmation("Are you sure?") is True
    assert "there's no default choice" in capsys.readouterr().err
    ...
    with PatchedAttribute(prompts, "interactive_prompt", lambda p: "y"):
        for default_value, expected_text in (
            ...
            assert (
                prompt_for_confirmation("Are you sure?", default=default_value) is True
            )
...
def test_ansi_style():
    ...
    assert ansi_style(background=214) == "%s48;5;214%s" % (ANSI_CSI, ANSI_SGR)
...
def test_html_to_ansi():
    ...
    # Bold text.
    assert (
        html_to_ansi("Let's try <b>bold</b>") == "Let's try \x1b[0m\x1b[1mbold\x1b[0m"
    )
    assert (
...
def test_generate_output(capsys):
    ...
    assertEqual(text + "\n", captured.out)
    assertEqual("", captured.err)


def test_generate_message(capsys):
    """Test the :func:`humanfriendly.terminal.message()` function."""
    text = "Standard error generated by message()"
    message(text)
    captured = capsys.readouterr()
    assertEqual("", captured.out)
    assertEqual(text + "\n", captured.err)


def test_generate_warning(capsys):
...
def ignore_coverage_warning(stream):
    """
...
def test_parse_usage_simple():
    """Test :func:`humanfriendly.usage.parse_usage()` (a simple case)."""
    introduction, options = preprocess_parse_result("""
...
def preprocess_parse_result(text):
...
def test_alias_proxy_deprecation_warning():
...
def test_alias_proxy_sphinx_compensation():
    """Test that the DeprecationProxy class emits deprecation warnings."""
...
def test_alias_proxy_sphinx_integration():
...
def test_sphinx_customizations():
    ...
    assert not docstring_is_reformatted(fake_app.bogus_usage)


def docstring_is_reformatted(entity):
